...
import sys
from unittest.mock import Mock, patch  # Mock used for _make_llm stubs
...
def test_make_llm_adds_base_url_for_ollama_models(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OLLAMA_REQUEST_TIMEOUT", "900")
...
def test_llm_for_role_prefers_role_specific_override(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_llm_for_role_falls_back_to_worker_model(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_default_ollama_base_url_uses_env_when_set(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OLLAMA_BASE_URL", "http://custom:11434")
    assert default_ollama_base_url() == "http://custom:11434"


def test_default_ollama_base_url_uses_127_on_windows_when_unset(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("OLLAMA_BASE_URL", raising=False)
    with patch.object(sys, "platform", "win32"):
        assert default_ollama_base_url() == "http://127.0.0.1:11434"


def test_default_ollama_base_url_uses_localhost_on_non_windows_when_unset(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("OLLAMA_BASE_URL", raising=False)
